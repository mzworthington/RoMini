...
A cover is a PNG, JPEG, GIF, or WebP stored beside the data volume as `covers/<library path>/cover.png` (or `.jpg`, `.gif`, `.webp`), outside `library/` so it is not treated as audio. The dashboard serves it at `/library/cover/<path>`. The library list and the live player use that URL, and `/logo.svg` when no cover file is stored. Set the picture from the row actions on that story.
