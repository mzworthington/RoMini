...
- As a listener, I want to tap a toy to start the story and tap that same toy again to pause (`tap`).
...
| FR-07 | Web Admin | `http://romini.local` on house WPA Wi-Fi: upload, map tags, Play mode, volume, free space, remaining charge from the UPS HAT. No HTTP PIN. | P1 |
