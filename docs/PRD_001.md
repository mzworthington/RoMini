...
| FR-06 | Physical controls | Four 16mm buttons: halt+LED, volume up, volume down, play/pause (long-press = restart). Software volume ceiling. | P0 |
| FR-07 | Web Admin | `http://romini.local` on house WPA Wi-Fi: upload, map tags, Play mode, free space. No battery. No HTTP PIN. | P1 |
...
```mermaid
flowchart LR
  toy["NTAG203 / Toy"] --> nfc["PN532 HAT SPI"]
...
- NFC + play/pause domain on `sim`, then PN532 SPI + mpv into the powered speaker.
- Presence grace and tap transport on fakes.

### Milestone 2: Enclosure & Controls

- Four 16mm buttons, LED, powered speaker aux from AV jack.
