...
Hardware why: [hardware.md](./hardware.md). Spec: [spec.md](./spec.md). Laptop: [development.md](./development.md).

Raspberry Pi OS **Lite 64-bit**. Hostname `romini`. User `pi` (or edit systemd `User=`).
...
Power off. Halt: COM GND, NO BCM 17, LED − GND, LED + BCM 27. Vol− BCM 22, vol+ BCM 23, play BCM 24 (NO to pin, COM GND). Enable `gpio-shutdown` on BCM 17 so the halt button can wake.
...
2. `fstab`: `LABEL=romini-data /var/lib/romini ext4 defaults 0 2` (no `nofail` once overlay is on).
...
House WPA from Imager. `avahi-daemon` for `http://romini.local`. Playback does not need the internet.
...
## 7. Application (when the wheel exists)

Clone into `/var/lib/romini/repo`, venv `/var/lib/romini/install/venv`, `ROMINI_PROFILE=pi`, `ROMINI_DATA=/var/lib/romini`. Enable `romini-core` and optional `romini-update.timer`.
